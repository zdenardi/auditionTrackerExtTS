...
if (filterList) {
    ...
    if (!noStudentFilmCheckBox.length) {
        const checkBox = $('<input>').addClass('checkbox').attr('id', 'no-student-films').attr('type', 'checkbox')
        const label = $('<label>').attr('for', 'no-student-films').text('Hide Student Films')
    ...
    if (!noVertShortForms.length) {
        const checkBox = $('<input>').addClass('checkbox').attr('id', 'no-vert-shorts').attr('type', 'checkbox')
        const label = $('<label>').attr('for', 'no-vert-shorts').text('Hide Vertical Shorts')
        filterList.append('<br>')
        filterList.append(checkBox)
        filterList.append(label)
    }
